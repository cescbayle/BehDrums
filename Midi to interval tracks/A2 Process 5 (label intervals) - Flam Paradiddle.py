...
# Load the csv file
file_path = '160_caltrack.mid_filtered_flam_time&velocity_with start and intervals.csv'
df = pd.read_csv(file_path)

# Calculate the group and interval indices
group_indices = np.ceil((df.index + 1) / 36).astype(int)
interval_indices = ((df.index % 36) + 1).astype(int)
